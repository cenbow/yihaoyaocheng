...
import com.yaoex.usermanage.model.erp.CustErpRelationVO;
...
public class OrderIssuedExceptionController extends BaseJsonController {
    ...
    @ResponseBody
    public Map<String, Object> relatedCustomers(@RequestBody CustErpRelationVO custErpRelationVO) throws Exception {
        UserDto userDto = super.getLoginUser();
        custErpRelationVO.setSeller_code(userDto.getCustId()+"");
        if (!UtilHelper.isEmpty(iCustErpRelationServiceDubbo)){
            return   iCustErpRelationServiceDubbo.createOrUpdateCustRelation(custErpRelationVO,userDto.getCustName());
        }else {
